...
export const ModalCerrarCaja = ({
  ...
  const hayDiferencia = montoContado !== '' && Math.abs(diferencia) >= 0.01;
        ...
        {resumen && (
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div className="flex justify-between p-2">
                <span className="text-gray-600">Otros medios de pago</span>
                <span className="font-medium">{formatCurrency(resumen.totalOtros ?? 0)}</span>
              </div>
              <div className="flex justify-between p-2">
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Observaciones{hayDiferencia && <span className="text-red-500"> *</span>}
            </label>
            <textarea
              value={observaciones}
              onChange={(e) => setObservaciones(e.target.value)}
              rows={2}
              placeholder={hayDiferencia ? 'Describa el motivo de la diferencia...' : 'Notas sobre el cierre de caja...'}
              className={`block w-full border rounded-lg px-3 py-2 text-sm
                         focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500
                         ${hayDiferencia && !observaciones.trim() ? 'border-red-400' : 'border-gray-300'}`}
            />
            {hayDiferencia && !observaciones.trim() && (
              <p className="text-xs text-red-500 mt-1">
                Las observaciones son obligatorias cuando hay diferencia en el arqueo
              </p>
            )}
          </div>
        </div>
